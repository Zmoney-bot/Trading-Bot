...
def load_signal_log() -> pd.DataFrame:
    signal_log = pd.DataFrame(columns=SIGNAL_COLUMNS)

    try:
        with sqlite3.connect(DATABASE_FILE) as connection:
            signal_log = pd.read_sql_query(
                "SELECT * FROM signals",
                connection,
            )
        ...
        if SIGNALS_FILE.exists():
            try:
                ...
                print("Loaded signal journal from CSV fallback.")
...
def save_signal_log(signal_log: pd.DataFrame) -> None:
    if signal_log.empty:
        print("Signal journal is empty. Save skipped.")
        return

    safe_log = signal_log.copy()
    ...
    for column in timestamp_columns:
        if column in safe_log.columns:
            safe_log[column] = safe_log[column].apply(
                lambda value: (
                    None
                    if pd.isna(value)
                    else value
    ...
    temporary_database = DATABASE_FILE.with_name(
        f"{DATABASE_FILE.stem}.tmp{DATABASE_FILE.suffix}"
    )
    ...
    try:
        ...
        safe_log.to_csv(SIGNALS_FILE, index=False)
    ...
    for index in pending_indices:
        ...
        for candle_timestamp, candle in future.iterrows():
            ...
            if stop_hit:
                ...
                resolved_timestamp = pd.Timestamp(candle_timestamp).isoformat()
                break
        ...
        if outcome is None and latest_timestamp >= expiry_timestamp:
            ...
            resolved_timestamp = pd.Timestamp(latest_timestamp).isoformat()
